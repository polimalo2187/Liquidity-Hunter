...
from app.plans import PLAN_FREE, PLAN_PLUS, PLAN_PREMIUM, activate_plus, activate_premium, extend_current_plan, can_access_visibility
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action.startswith("view_signals_tier:"):
            requested_visibility = action.split(":", 1)[1]
            await handle_view_signals_tier(query, user, admin, users_col, requested_visibility)
            return
...
# ======================================================
# HANDLER VIEW SIGNALS (MENÚ POR NIVEL)
# ======================================================

def _signal_visibility_meta(visibility: str) -> tuple[str, str]:
    normalized = str(visibility or "").lower()
    if normalized == PLAN_PREMIUM:
        return "🔴", "PREMIUM"
    if normalized == PLAN_PLUS:
        return "🟡", "PLUS"
    return "🟢", "FREE"


def _signal_tier_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🟢 Free", callback_data=f"view_signals_tier:{PLAN_FREE}")],
        [InlineKeyboardButton("🟡 Plus", callback_data=f"view_signals_tier:{PLAN_PLUS}")],
        [InlineKeyboardButton("🔴 Premium", callback_data=f"view_signals_tier:{PLAN_PREMIUM}")],
        [InlineKeyboardButton("⬅️ Volver al menú", callback_data="back_menu")],
    ])


def _signal_tier_back_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Volver a señales", callback_data="view_signals")],
        [InlineKeyboardButton("⬅️ Volver al menú", callback_data="back_menu")],
    ])
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        await query.edit_message_text(
            (
                "🚨 SEÑALES EN VIVO\n\n"
                "Selecciona el nivel de señal que quieres consultar:\n"
                "• 🟢 Free\n"
                "• 🟡 Plus\n"
                "• 🔴 Premium\n\n"
                f"Tu plan actual: {str(plan).upper()}"
            ),
            reply_markup=_signal_tier_menu(),
        )
        ...
        await query.edit_message_text(
            "❌ Error al abrir el menú de señales.",
            reply_markup=back_to_menu(),
        )


async def handle_view_signals_tier(query, user, admin, users_col, requested_visibility: str):
    try:
        user_id = user["user_id"]
        plan = PLAN_PREMIUM if admin else user.get("plan", PLAN_FREE)
        requested_visibility = str(requested_visibility or "").lower()

        if requested_visibility not in {PLAN_FREE, PLAN_PLUS, PLAN_PREMIUM}:
            await query.edit_message_text(
                "❌ Categoría de señal no válida.",
                reply_markup=_signal_tier_back_menu(),
            )
            return
        ...
        if not can_access_visibility(plan, requested_visibility, admin=admin):
            icon, label = _signal_visibility_meta(requested_visibility)
            await query.edit_message_text(
                (
                    f"🔒 No tienes acceso a señales {label}.\n\n"
                    f"{icon} Esta categoría requiere un plan superior."
                ),
                reply_markup=_signal_tier_back_menu(),
            )
            return

        user_signals = get_latest_base_signal_for_plan(user_id, requested_visibility)
        if not user_signals:
            icon, label = _signal_visibility_meta(requested_visibility)
            await query.edit_message_text(
                f"📭 Ahora mismo no hay señales {label} activas.",
                reply_markup=_signal_tier_back_menu(),
            )
            return
        ...
        logger.error(f"Error en handle_view_signals_tier: {e}", exc_info=True)
        await query.edit_message_text(
            "❌ Error al obtener la señal solicitada.",
            reply_markup=_signal_tier_back_menu(),
        )
