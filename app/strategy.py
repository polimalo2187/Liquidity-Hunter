...
PREMIUM_PROFILE = {
    ...
    "min_rr": 1.70,
    "min_barrier_rr": 1.20,
    "ema_reclaim_buffer_atr": 0.10,
    "max_countertrend_atr": 0.20,
    "min_pivots": 2,
...
PLUS_PROFILE = {
    ...
    "min_rr": 1.55,
    "min_barrier_rr": 1.05,
    "ema_reclaim_buffer_atr": 0.06,
    "max_countertrend_atr": 0.30,
    "min_pivots": 2,
...
def _confirmation_candle_ok(confirm_candle: pd.Series, sweep_candle: pd.Series, direction: str, profile: Dict) -> bool:
    ...
    if float(confirm_candle["rel_volume"]) < float(profile["min_confirm_rel_volume"]):
        return False
...
def _higher_timeframe_context_ok(df_1h: pd.DataFrame, direction: str, profile: Dict) -> bool:
    ...
    if direction == "LONG":
        if close < ema50 - countertrend:
            return False
        if ema20 < ema50 and slope20 <= 0:
            return False
        if close < ema20 - countertrend:
            return False
        return True
...
def _nearest_barrier_price(
    ...
    if direction == "LONG":
        ...
        for value in [
            float(historical.iloc[-1]["ema20"]),
            float(df_1h.iloc[-2]["ema20"]),
            float(df_1h.iloc[-2]["ema50"]),
        ]:
