...
PLAN_ACCESS = {
    PLAN_FREE: {PLAN_FREE},
    PLAN_PLUS: {PLAN_FREE, PLAN_PLUS},
    PLAN_PREMIUM: {PLAN_FREE, PLAN_PLUS, PLAN_PREMIUM},
}
...
def allowed_visibilities_for_plan(plan: Optional[str], admin: bool = False) -> set[str]:
    """
    Retorna los niveles de señal visibles para el plan del usuario.
    """
    if admin:
        return set(PLAN_ACCESS[PLAN_PREMIUM])

    normalized_plan = str(plan or PLAN_FREE).lower()
    return set(PLAN_ACCESS.get(normalized_plan, PLAN_ACCESS[PLAN_FREE]))


def can_access_visibility(plan: Optional[str], visibility: str, admin: bool = False) -> bool:
    """
    Verifica si un plan puede acceder a una categoría de señal.
    """
    return str(visibility or "").lower() in allowed_visibilities_for_plan(plan, admin=admin)
